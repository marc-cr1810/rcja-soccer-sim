...
import { foldTournament, parseAmendment, type Amendment } from '../../src/accounts/amendments';
import { parseKickoff } from '../../src/infra/cli';
...
import {
  ...
  saveDraw,
  saveResult,
} from '../../src/league/tournament-store';
...
describe('amend schedule, from a terminal', () => {
  ...
  async function runCli(args: string[]): Promise<{ code: number; out: string }> {
    ...
    const proc = Bun.spawn(['bun', cliPath, ...args, '--tournaments', dir, '--data', join(dir, 'league')], {
      cwd: dir,
      stdout: 'pipe',
      stderr: 'pipe',
    });
