...
import { splitArenaPath, type ServerOptions } from '../infra/server';
import { ArenaSupervisor } from './arenas';
import { AGENT_PATH } from '../infra/gateway';
import { Occupancy, numberOfSeat, type Placement } from '../accounts/occupancy';
import { Tenancy } from '../accounts/tenancy';
...
import { loadSettings, mergeSettings, saveSettings, type LeagueSettings, type SettingSource } from '../infra/settings';
import {
  decided,
  wholeMinutes,
  NO_PENALTIES,
...
import { totalUsage } from '../infra/usage';
import { Accounts, type Account } from '../accounts/accounts';
import {
  CAPABILITIES,
  SCOPES,
  can,
  ...
  type Actor,
  type Capability,
} from '../accounts/capabilities';
import { bearer, type Authority, type AuthRequest, type Submitter } from '../accounts/authority';
import { parseManifest, slugifyTeam, TOKEN_FILENAME } from '../infra/manifest';
import { listPushes, readPush } from '../match/pushes';
import { hashSubmission } from '../accounts/submission';
import { resolveLineup, type LineupEntry } from '../accounts/lineup';
import {
  deriveTable,
  fixtureOutcome,
  nextFixture,
...
import type { Amendment, DraftAmendment } from '../accounts/amendments';
...
export class LeagueServer {
  ...
  private async draftAmendment(
    ...
    return { ok: false, reason: 'unknown amendment kind' };
  }
