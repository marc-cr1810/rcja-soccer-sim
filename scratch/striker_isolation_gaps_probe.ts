...
async function playSeed(seed: number): Promise<{ h1: number; h2: number; gaps: {half:1|2;at:number;sinceLast:number}[] }> {
  ...
  const child = spawn(`python3 python/examples/play.py --only cyan --url ${url}`, {
    shell: true,
    stdio: 'ignore',
    detached: true,
  });
  try {
    ...
    let h2 = 0;
    let prevTotal = 0;
    const gaps: { half: 1 | 2; at: number; sinceLast: number }[] = [];
    let lastGoalT = 0;
    const observe = (m: { world: { half: 1 | 2; clock: number; score: { cyan: number; yellow: number } } }): void => {
      const total = m.world.score.cyan + m.world.score.yellow;
      if (total > prevTotal) {
        if (m.world.half === 1) h1++;
        else h2++;
        const sinceLast = m.world.half === 1 ? m.world.clock - 0 : 0;
        gaps.push({ half: m.world.half, at: m.world.clock, sinceLast: lastGoalT === 0 ? m.world.clock : m.world.clock - lastGoalT });
        lastGoalT = m.world.clock;
      }
      prevTotal = total;
    };
    await server.play({
      agents: { 'yellow-1': statue, 'yellow-2': statue } as never,
...
async function main(): Promise<void> {
  ...
  for (let seed = 1; seed <= SEEDS; seed++) {
    const { h1, h2, gaps } = await playSeed(seed);
    h1Total += h1;
    h2Total += h2;
    console.log(`seed ${seed}: h1=${h1} h2=${h2}  gaps=${gaps.map(g=>g.half+g.sinceLast.toFixed(1)).join(' ')}`);
  }
  console.log(`\nTOTAL over ${SEEDS} seeds: h1=${h1Total}  h2=${h2Total}`);
  console.log(
    `h1 attacks +x (yellow goal); h2 attacks -x (cyan goal), after the half-time end swap.`,
  );
  process.exit(0);
}
