"""MicroPython's `uerrno` - CPython's `errno` under the name a board uses.

On a real board `uerrno` and `errno` are the same module; the `u` prefix is
...
one implementation, and the simulator never has a subtly different `errno`
of its own to keep in step.
"""

import sys as _sys

import errno as _impl

#: The import machinery hands back whatever is in `sys.modules` once this
#: file finishes, so this *is* the module - `import uerrno; uerrno is errno` holds,
