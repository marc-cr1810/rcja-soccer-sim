...
#: Rule 5.4.7 asks for 50 mm; this is past it by enough that a position estimate
#: still settling after the restart cannot call it early.
...
def think(s, me):
    ...
    # Hold the post while the kick-off is actually live, and not a moment
    # longer. `kickoff.pending` is a three-second ceiling off the start button,
    # not a referee's all-clear - nothing sends one - so a keeper that waits it
    # out at every restart is parked for two of the match's three minutes and
    # concedes the difference. The ball leaving the centre spot is the signal.
...
def say(me, state: str, distance: float | None = None) -> None:
    ...
    if ticks % 25 == 0:
        ...
        print(
            f"[{TEAM}-{NUMBER} keeper ] {state:<14} {extra}"
