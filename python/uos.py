"""MicroPython's `uos` - CPython's `os` under the name a board uses.

On a real board `uos` and `os` are the same module; the `u` prefix is
...
one implementation, and the simulator never has a subtly different `os`
...
#: file finishes, so this *is* the module - `import uos; uos is os` holds,
