"""MicroPython's `uio` - CPython's `io` under the name a board uses.

On a real board `uio` and `io` are the same module; the `u` prefix is
...
one implementation, and the simulator never has a subtly different `io`
...
#: file finishes, so this *is* the module - `import uio; uio is io` holds,
