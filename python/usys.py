"""MicroPython's `usys` - CPython's `sys` under the name a board uses.

On a real board `usys` and `sys` are the same module; the `u` prefix is
...
one implementation, and the simulator never has a subtly different `sys`
...
#: file finishes, so this *is* the module - `import usys; usys is sys` holds,
