"""MicroPython's `uzlib` - CPython's `zlib` under the name a board uses.

On a real board `uzlib` and `zlib` are the same module; the `u` prefix is
...
one implementation, and the simulator never has a subtly different `zlib`
of its own to keep in step.
"""

import sys as _sys

import zlib as _impl

#: The import machinery hands back whatever is in `sys.modules` once this
#: file finishes, so this *is* the module - `import uzlib; uzlib is zlib` holds,
