...
import { appendAmendment, loadAmendments, saveDraw, saveResult } from '../src/tournament-store';
import type { FixtureResult } from '../src/tournament';
...
async function resultTime(started: Started, fixtureId: string): Promise<string> {
  const { loadDraw, loadResults } = await import('../src/tournament-store');
