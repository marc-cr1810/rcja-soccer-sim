...
describe('parseManifest', () => {
  it('accepts a well-formed manifest naming a file that was pushed', () => {
    const result = parseManifest(
      json({ team: 'ACT-01', robot: 1, entry: 'striker.py' }),
      new Set(['manifest.json', 'striker.py']),
    );
    expect(result).toEqual({ ok: true, value: { team: 'ACT-01', robot: 1, entry: 'striker.py' } });
  });
  ...
  it('rejects a robot number other than 1 or 2', () => {
    const result = parseManifest(json({ team: 'ACT-01', robot: 3, entry: 'a.py' }), new Set(['a.py']));
    expect(result.ok).toBe(false);
  });

  it('rejects an entry that was not among the pushed files', () => {
    const result = parseManifest(
      json({ team: 'ACT-01', robot: 1, entry: 'missing.py' }),
  ...
  it('rejects an entry with a subdirectory', () => {
    const result = parseManifest(
      json({ team: 'ACT-01', robot: 1, entry: 'sub/dir.py' }),
      new Set(['sub/dir.py']),
    );
    expect(result.ok).toBe(false);
  });
...
describe('slugifyTeam', () => {
  it('lowercases and collapses unsafe characters', () => {
    expect(slugifyTeam('ACT-01 (Rebels)!')).toBe('act-01-rebels');
  });
