...
import { bwrapAvailable, cgroupAvailable, sandboxUnavailableReason } from './sandbox';
import { SEATS_PER_ARENA, type LeagueSettings, type SettingSource } from './settings';
...
export function resolveBudget(settings: LeagueSettings, machine: Machine): Budget {
  ...
  const max = settings.arenas.max ?? guaranteed;
...
export function formatCapacity(
  ...
  lines.push('');
  lines.push(
    `  configured           arenas.max ${budget.set ? budget.max : `${budget.max} (computed)`}${where('arenas.max')}`,
  );
  lines.push(
