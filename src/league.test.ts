...
import { LeagueServer } from './league';
...
describe('the areas a visitor may not enter', () => {
  it('refuses the referee console, including its static files', async () => {
    ...
    const action = await call(port, '/referee-api/kickoff', { method: 'POST' });
  ...
  it('refuses a team the referee console', async () => {
    const started = await start();
    const { cookie } = await aTeam(started);
    const answer = await call(started.port, '/referee-api/kickoff', { method: 'POST', cookie });
    expect(answer.status).toBe(403);
  });
});
